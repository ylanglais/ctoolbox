...
int
dstsrv_srv_cli_dialog(int srvnum, int cid, void *data) {
	ppayload_t pl;
	char msg[4096];
	...
	if (srv_recv_to(cid, msg, 1046, _dstsrv_.timeout) <= 0) {
	...
	while (1) {
		if (state == LISTEN) {
			...
			if (!strncmp(msg, "PING", 4)) {
				...
				if (srv_send_to(cid, "PONG", 5, _dstsrv_.timeout) <= 0) {
			...
			if (srv_send_to(cid, cmdl, strlen(cmdl) + 1, _dstsrv_.timeout) <=0) { 
...
void dstsrv_loop() {
	srv_cli_connect_loop(_dstsrv_.srv, dstsrv_srv_cli_dialog, dstsrv_must_stop, NULL, _dstsrv_.cnxwait);
}
...
main(int n, char *a[]) {
	...
	/* server creation : */
	if (!(_dstsrv_.srv = srv_new(_dstsrv_.port, _dstsrv_.maxclient))) {
