...
char tstmr_MODULE[]  = "Timstamps and timers";
char tstmr_PURPOSE[] = "Timstamps and timers";
char tstmr_VERSION[] = "1.2";
char tstmr_DATEVER[] = "04/08/2016";
...
tstamp_div(tstamp_t ts1, int k) {
	...
	if (k == 0) {
		err_warning("cannot divide by 0");
		return ts;
	}
	ts.tv_sec   =  ts1.tv_sec  / k;
	ts.tv_usec  =  ts1.tv_usec / k;
	ts.tv_usec += (ts1.tv_sec % k) * 1000000 / k;
...
tstamp_fmt(char *buff, tstamp_t ts) {
	struct tm *t;
	*buff = 0;
	if (ts.tv_sec == 0 && ts.tv_usec == 0) return buff;
	t = localtime(&ts.tv_sec);
	sprintf(buff, "%04d/%02d/%02d %02d:%02d:%02d.%03d", 
		t->tm_year + 1900, t->tm_mon + 1, t->tm_mday, t->tm_hour, t->tm_min, t->tm_sec, (int) ts.tv_usec / 1000);
	return buff;
}

char *
tstamp_fmt_fr(char *buff, tstamp_t ts) {
	struct tm *t;
	*buff = 0;
	if (ts.tv_sec == 0 && ts.tv_usec == 0) return buff;
	t = localtime(&ts.tv_sec);
	sprintf(buff, "%02d/%02d/%04d %02d:%02d:%02d.%03d", 
		t->tm_mday, t->tm_mon + 1, t->tm_year + 1900, t->tm_hour, t->tm_min, t->tm_sec, (int) ts.tv_usec / 1000);
	return buff;
}

char *
tstamp_fmt_ordered(char *buff, tstamp_t ts) {
	struct tm *t;
	*buff = 0;
	if (ts.tv_sec == 0 && ts.tv_usec == 0) return buff;
	t = localtime(&ts.tv_sec);
	sprintf(buff, "%04d%02d%04d%02d%02d%02d.%03d", 
		t->tm_year + 1900, 
		t->tm_mon + 1, 
		t->tm_mday, 
		t->tm_hour, 
		t->tm_min, 
		t->tm_sec, (int) ts.tv_usec / 1000);
	return buff;
}

char *
tstamp_fmt_db(char *buff, tstamp_t ts) {
	struct tm *t;
	*buff = 0;
	if (ts.tv_sec == 0 && ts.tv_usec == 0) return buff;
	t = localtime(&ts.tv_sec);
	sprintf(buff, "%04d%02d%04d%02d%02d%02d", 
		t->tm_year + 1900, 
		t->tm_mon + 1, 
		t->tm_mday, 
		t->tm_hour, 
...
tstamp_duration_fmt(char *buff, tstamp_t ts) {
	...
	if (t->tm_year > 70) 
		...
		t->tm_year - 70, t->tm_mon, t->tm_mday - 1, t->tm_hour - 1, t->tm_min, t->tm_sec, (int) ts.tv_usec / 1000);
	else if (t->tm_mon > 0) 
		sprintf(buff, "%d months %d days %02d:%02d:%02d.%03d", 
		t->tm_mon, t->tm_mday - 1, t->tm_hour - 1, t->tm_min, t->tm_sec, (int) ts.tv_usec / 1000);
	else if (t->tm_mday > 1) 
		sprintf(buff, "%d days %02d:%02d:%02d.%03d", 
		t->tm_mday - 1, t->tm_hour - 1, t->tm_min, t->tm_sec, (int) ts.tv_usec / 1000);
	else 
		sprintf(buff, "%02d:%02d:%02d.%03d", 
		t->tm_hour - 1, t->tm_min, t->tm_sec, (int) ts.tv_usec / 1000);
...
tmr_dump(ptmr_t t) {
	...
	printf("start  : %s\n", tstamp_fmt(b, t->start));
	printf("elapsed: %s\n", tstamp_duration_fmt(b, t->elapsed));
} 
...
main(void) {
	...
	t = tmr_new();
	tmr_start(t);
	while (gets(b)) printf("%s: %s\n", tstamp_duration_fmt(tb, tmr_laps(t)), b);
	tmr_destroy(t);
	return 0;
}
#endif
