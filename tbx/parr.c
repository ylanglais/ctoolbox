...
/*    
	...
	Todo:
		- Clean up page activity from parr activity, especially page loading & data pointer fix after load
		- Need locking for multi {thread,process}ing
		- Test concurrency
...
static void 
_parr_lock(pparr_t arr) {
	if (!arr) return;
	while (arr->lock);
	arr->lock = 1;
}
static void 
_parr_unlock(pparr_t arr) {
	if (!arr) return;
	arr->lock = 0;
}
	...
	if (futl_is_dir(path) && futl_is_file(fname)) {
		if ((arr = (pparr_t)  pmalloc(fname, sizeof(parr_t)))) {
			...
			for (i = 0; i < arr->npages; i++) {
				...
				if (!(page = (ppage_t) pmalloc(pname, _parr_page_size(arr)))) {
					/** Error handling **/	
...
parr_destroy(pparr_t arr) {
	if (arr) {
		...
		if (arr->idx) arr->idx = pfree(arr->idx);
...
_parr_lock(arr);
	...
	/* squeeze index: */
	arr->used--;
	memmove((char *) (i - 1), (char *) i, (arr->used - index) * sizeof(slot_t));
