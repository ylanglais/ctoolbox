...
nqs_srv_put_request(pnqs_t nqs, pmsg_t msg) {
	...
	while (1) {
		...
		if (crc == msg_put_crc(msg)) {
			...
			pqe_t qe;
			if (!(qe = qe_new(msg_put_data(msg),  msg_put_size(msg), tstamp_get(), msg_put_crc(msg), srv_cli_connected_host(nqs->srv)))) {
				err_debug("message_put, cannot create q entry");
				err = merr_BAD_QUEUE_ENTRY;
				goto cleanup;
			}

			if ((qerr = nq_put(nqs->nq, qe)) == nqerr_NONE) {
				err_debug("message put, pushed queue entry");
...
nqs_srv_get_request(pnqs_t nqs, pmsg_t msg) {
	...
	if ((qerr = nq_get(nqs->nq, &qe)) != nqerr_NONE) {
