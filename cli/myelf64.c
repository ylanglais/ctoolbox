...
elf_new(char *filename) {
    ...
    if (!(e->allsheaders = (Elf64_Shdr **) malloc(sizeof(Elf64_Shdr *) * (e->ehdr->e_shnum + 1)))) {
        fprintf(stderr, "cannot allocate memory for section headers pointers\n");
        return elf_destroy(e);
    }

    p = (Elf64_Shdr *) ((char *) e->edat->d_buf + e->ehdr->e_shoff);
...
/* Given Elf header, Elf_Scn, and Elf64_Shdr 
 * print out the symbol table 
 */
void
print_symbols(Elf * elf, Elf_Scn * scn, Elf64_Shdr * shdr) {
    Elf64_Sym *esym, *lastsym;
...
main(int nargs, char **args) {
    ...
    while ((section = elf_nextscn(e->elf, section))) {
        if (!(sheader = elf64_getshdr(section)))
            continue;
...
#if 0
            /* get string section: */
            sym = (Elf64_Sym *) ((char *) e->edat->d_buf) + sheader->sh_offset;
            endsym =
                (Elf64_Sym *) ((char *) e->edat->d_buf) + sheader->sh_offset +
                sheader->sh_size;

            for (; sym < endsym; sym++) {
                printf("type = %-8si, size = %d, name = %s\n",
                       tt[ELF64_ST_TYPE(sym->st_info)], sym->st_size,
                       strings + sym->st_name);
            }
#endif
        }
